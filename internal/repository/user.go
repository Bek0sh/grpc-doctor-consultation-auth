...
func (r *repo) CreateUser(userInput *models.RegisterUser) (int, error) {
	...
	query := "INSERT INTO users (name, surname, phone_number, hashed_password) VALUES($1, $2, $3, $4) RETURNING id"
	err := r.db.QueryRow(query, userInput.Username, userInput.Surname, userInput.PhoneNumber, userInput.Password).Scan(&id)
	if err != nil {
...
func (r *repo) GetUserById(id int) (*models.UserResponse, error) {
	...
	query := "SELECT name, surname, phone_number FROM users WHERE id=$1"

	err := r.db.QueryRow(query, &id).Scan(&response.Username, &response.Surname, &response.PhoneNumber)
...
func (r *repo) GetUserByPhoneNumber(phoneNumber string) (*models.User, error) {
	...
	query := "SELECT id, name, surname, phone_number, hashed_password FROM users WHERE phone_number=$1"

	err := r.db.QueryRow(query, &phoneNumber).Scan(&response.Id, &response.Username, &response.Surname, &response.PhoneNumber, &response.Password)
	if err != nil {
